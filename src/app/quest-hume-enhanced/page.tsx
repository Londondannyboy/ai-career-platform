...
export default function QuestHumeEnhancedPage() {
  ...
  const processVoiceInput = async (transcript: string) => {
    ...
    try {
      // Load custom prompts with enhanced Hume voice instructions
      ...
      console.log('🤖 Using enhanced prompt for Hume voice')
      if (wasInterrupted) {
      ...
      const response = await fetch('/api/hume-clm', {
        ...
        body: JSON.stringify({
          ...
          user_id: userId,
          custom_session_id: `quest_hume_enhanced_${userId}_${Date.now()}`,
          emotional_context: { 
            engagement: 0.9, 
            conversation_mode: 'hume_voice',
            platform: 'quest_hume_enhanced',
    ...
    try {
      ...
      if ('webkitSpeechRecognition' in window) {
        ...
        recognition.onstart = async () => {
          ...
          setLastResponse('🎤 Quest AI with enhanced voice is listening... speak naturally!')
        ...
        recognition.onresult = async (event: any) => {
          ...
          if (isSpeakingRef.current && 'speechSynthesis' in window && transcript.length > 2) {
            console.log('🛑 Auto-interrupting enhanced voice - user voice detected')
            speechSynthesis.cancel()
            setIsSpeaking(false)
            isSpeakingRef.current = false
            setWasInterrupted(true)
          ...
          if (isFinal && transcript.length > 0) {
            console.log('🗣️ User said (enhanced):', transcript)
  ...
  const interruptConversation = () => {
    ...
    isSpeakingRef.current = false
    setIsProcessing(false)
    setLastResponse('🔄 Enhanced conversation interrupted. Listening for new input...')
  }

  return (
    <div className="container mx-auto p-6 max-w-4xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Quest AI - Enhanced Voice with Hume Quality</h1>
        <p className="text-muted-foreground">
          Professional voice conversation with enhanced audio quality and natural interruption handling
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>Enhanced Voice Assistant</CardTitle>
            <CardDescription>
              High-quality voice with professional speech synthesis and advanced interruption detection
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Button 
                ...
                size="lg"
              >
                {isRecording ? '🛑 Stop Enhanced Quest AI' : '🎤 Start Enhanced Quest AI'}
              </Button>
